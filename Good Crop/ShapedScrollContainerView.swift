...
//    }
    ...
    var anchorPoint: CGPoint? {
        didSet {
            print("anchorPoint did set (ShapedScrollContainerView)")
...
//        println("shapeLayer.frame = \(shapeLayer.frame)")
    ...
    private func pathForShape() -> CGPath
    {
        ...
        path2.lineJoinStyle = CGLineJoin.Round
...
//        }
        let anchorPoint = self.anchorPoint ?? CGPointZero
...
//            let tan_a_2 = min(tan(angle / 2), 1)
            ...
            if tag == 2 {
                ...
                print("y = \(y)")
                print("point0.y - y = \(point0.y - y)")
                print("anchorPoint.y - Y - spacing.fl = \(anchorPoint.y - Y - spacing.fl)")
                print("")
                print("(offset3 - spacing) = \((offset3 - spacing))")
                print("")
        ...
        case .Five:
            ...
            // Angles
            /// Top left, left angle
            let angle1 = atan((anchorPoint.x.dbl - dx) / (anchorPoint.y.dbl - dx))
//            if angle1.isNaN || angle1.isInfinite { angle1 = 0.0 }
            /// Bottom left, left angle
            let angle2 = atan((anchorPoint.x.dbl - dx) / (Y.dbl - anchorPoint.y.dbl - dx))
//            if angle2.isNaN || angle2.isInfinite { angle2 = 0.0 }
            /// Top right, right angle
            let angle3 = atan((X.dbl - anchorPoint.x.dbl - dx) / (Y.dbl - anchorPoint.y.dbl - dx))
//            if angle3.isNaN || angle3.isInfinite { angle3 = 0.0 }
            /// Bottom right, right angle
            let angle4 = atan((X.dbl - anchorPoint.x.dbl - dx) / (anchorPoint.y.dbl - dx))
//            if angle4.isNaN || angle4.isInfinite { angle4 = 0.0 }
            ...
            let offsetLV1 = findCornerOffset(angle: angle1, dx: dx)
            let offsetLV2 = findCornerOffset(angle: angle2, dx: dx)
...
//            var offsetLH3 = (dx * tan(angle1 / 2) + dx * tan(angle2 / 2)) / (tan(M_PI / 2 - angle1) * tan(M_PI / 2 - angle2))
            let offsetL3 = findOffset(angle1: M_PI/2 - angle2, angle2: M_PI/2 - angle1, dx: dx)
            
            let offsetBH1 = findCornerOffset(angle: M_PI/2 - angle2, dx: dx) //dx * tan(angle2) + dx / cos(angle2) - dx
            let offsetBH2 = findCornerOffset(angle: M_PI/2 - angle3, dx: dx) // dx * tan(angle3) + dx / cos(angle3) - dx
...
//            var offsetBV3 = (dx * tan((M_PI / 2 - angle2) / 2) - dx * tan((M_PI / 2 - angle3) / 2)) / (tan(angle2) * tan(angle3))
            let offsetB3 = findOffset(angle1: angle3, angle2: angle2, dx: dx)
            
            let offsetRV2 = findCornerOffset(angle: angle3, dx: dx) // dx / tan(angle3) + dx / sin(angle3) - dx
            let offsetRV3 = findCornerOffset(angle: angle4, dx: dx) // dx / tan(angle4) + dx / sin(angle4) - dx
            let offsetR1 = findOffset(angle1: M_PI/2 - angle4, angle2: M_PI/2 - angle3, dx: dx)
            
            let offsetUH1 = findCornerOffset(angle: M_PI/2 - angle1, dx: dx) //dx * tan(angle1) + dx / cos(angle1) - dx
            let offsetUH3 = findCornerOffset(angle: M_PI/2 - angle4, dx: dx) //dx * tan(angle1 / 2) - dx * tan(angle4 / 2)
            let offsetU2 = findOffset(angle1: angle1, angle2: angle4, dx: dx) // dx * tan(angle4) + dx / cos(angle4) - dx
...
//            break
    ...
    private func findOffset(angle1 angle1: Double, angle2: Double, dx: Double) -> CGPoint
    {
    ...
    private func findCornerOffset(angle angle: Double, dx: Double) -> CGFloat
    {
