...
class ViewController: UIViewController, SwiftColorPickerDelegate, UIPopoverPresentationControllerDelegate, FrameViewDelegate, BackgroundViewDelegate, CollageViewDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UIScrollViewDelegate, ADBannerViewDelegate, GADBannerViewDelegate, GADInterstitialDelegate
{
    // MARK: - Outlets
...
//    }
    ...
    var images = [UIImage?]() {
        didSet {
            ...
            for image in images {
                if let _ = image { blinkingInformation = nil }
            }
...
//            l.borderWidth = newValue
    ...
    var aspectRatio: CGFloat {
        ...
        set {
            print("aspectRatio setter = \(newValue)")
            _aspectRatio = newValue
            
            print("(aspectRatio setter) newValue = \(newValue)")
            print("(aspectRatio setter) aspectRatio = \(aspectRatio)")
    ...
    var frameType: Int {
        ...
        set {
            if let selectedCompositionType = CompositionType(rawValue: newValue) {
                ...
                print("compositionType = \(compositionType)")
...
//        mainView.layer.shadowPath = shadowPath.CGPath
    ...
    func addGestureRecognizersForWrapperViews()
    {
        for wrapperView in collageView.wrapperViews {
            ...
            print("gesture recognizers added")
    ...
    func adjustSize()
    {
        ...
        // maybe collageView.bounds.width - spacing / 2 ?
            collageView.anchorPointRelative = CGPointMake(collageView.anchorPoint.x / collageView.bounds.width, collageView.anchorPoint.y / collageView.bounds.height)
        
        // Setup container
        let containerMargins: CGFloat = 10.0
        let containerBottomFrameHeight: CGFloat = clippedHeight! == 0 ? 100 : clippedHeight!
...
//            if self.image != nil { self.adjustImageSize(saveZoomScale: saveZoomScale) }
        ...
        print("2 collageView.anchorPointRelative = \(collageView.anchorPointRelative)")
    ...
    func selectFrame(gesture: UIGestureRecognizer)
    {
        ...
        case .Remove(let removeIt):
            let (_, errorMessage) = removeIt(view.tag)
        ...
        case .Switch(let switchIt):
            ...
            print("selectedViewTags = \(selectedViewTags)")
            view.alpha = 0.3
...
//            }, completionHandler: { success, error in
//                print("added image to album", terminator: "")
...
//        selectSize.popoverPresentationController!.sourceRect = self.navigationItem.rightBarButtonItem.
    ...
    - parameter scaleFactor:: Variable used to adjust the quality and also the size of the image to be shared. Default value is 4.0
    ...
    */
    func share(scaleFactor: CGFloat = 4.0)
    {
        ...
        let anchorViewVisibility = collageView.anchorView.hidden
        collageView.anchorView.hidden = true
        for wrapperView in collageView.wrapperViews {
...
//                    }
                    ...
                    self.collageView.anchorView.hidden = anchorViewVisibility
...
//    }
    ...
    override func viewDidLoad() {
        ...
        googleAdBannerView.adUnitID = "ca-app-pub-3881481313916870/7237974741"
...
//        interstitial.delegate = self
        ...
        // simulator.
        request.testDevices = [
        "CC29ECE9-2066-49A8-A378-3F650445705"
        ]
...
//        googleAdBannerView.hidden = true
        
    }
    func bannerView(banner: ADBannerView!, didFailToReceiveAdWithError error: NSError!) {
        adBannerView.hidden = true
//        googleAdBannerView.hidden = false
    }
    
    // MARK: - Google Interstitial Ad
    func createAndLoadAd() -> GADInterstitial
    {
        let ad = GADInterstitial(adUnitID: "ca-app-pub-3881481313916870/5761241540")
...
//            frameView.setTranslatesAutoresizingMaskIntoConstraints(false)
            ...
            collageView.anchorView.hidden = false
//            for wrapperView in collageView.wrapperViews {
...
//            change += translation.y
        ...
        case .Ended:
            ...
            if change >= 50 {
                ...
                panView.hide(0.6 * NSTimeInterval(ratio))
                clippedHeight = 0
...
//    }
    ...
    {
        print("unwind from photo gallery")
        if assets.count > 0 {
